...
def _compile_gemm(
    ...
    epi_args = GemmCls.EpilogueArguments(
        ...
        split_k_workspace=(
            # (cta_tile_m * cta_tile_n, ntile_m, ntile_n, L) f32 partials: one
            # opaque fragment-order region per tile (frag_stripe_view).
            fake_tensor(
                Float32,
...
def _split_k_buffers(D, split_k_mode, tile_M, tile_N, cluster_M, cluster_N, sm100):
    ...
    deterministic; PARALLEL: arrival counter, not deterministic) plus an opaque f32
    region per tile holding the non-finalizing splits' raw accumulator partials in
    the writer's fragment order (frag_stripe_view — reader is the writer, so the
    region has no (m, n) geometry). The scheduler's CTA tile ids are
    cluster-rounded (a boundary cluster can contain CTAs whose tile is fully out
    of bounds but which still run the protocol), so tile counts are rounded up to
    cluster multiples. The per-CTA tile M must match the kernel exactly (the
    region size is cta_tile_m * tile_N): SM100/110 2-CTA MMA (cluster_M even,
    tile_M 128/256) halves it.
    """
    num_l, len_m, len_n = D.shape
